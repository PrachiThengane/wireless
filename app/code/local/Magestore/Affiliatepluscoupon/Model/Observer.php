...
class Magestore_Affiliatepluscoupon_Model_Observer {
    ...
    public function couponPostActionOneStep($observer) {
		...
		$shippingMethod = $layout->createBlock('checkout/onepage_shipping_method_available')
		...
		$paymentMethod = $layout->createBlock('checkout/onepage_payment_methods','choose-payment-method')
		...
		if(Mage::helper('onestepcheckout')->isEnterprise() && Mage::helper('customer')->isLoggedIn()){
            ...
            $customerBalanceBlock = $layout->createBlock('enterprise_customerbalance/checkout_onepage_payment_additional', 'customerbalance', array('template'=>'onestepcheckout/customerbalance/payment/additional.phtml'));
            $customerBalanceBlockScripts = $layout->createBlock('enterprise_customerbalance/checkout_onepage_payment_additional', 'customerbalance_scripts', array('template'=>'onestepcheckout/customerbalance/payment/scripts.phtml'));

            $rewardPointsBlock = $layout->createBlock('enterprise_reward/checkout_payment_additional', 'reward.points', array('template'=>'onestepcheckout/reward/payment/additional.phtml', 'before' => '-'));
            $rewardPointsBlockScripts = $layout->createBlock('enterprise_reward/checkout_payment_additional', 'reward.scripts', array('template'=>'onestepcheckout/reward/payment/scripts.phtml', 'after' => '-'));

            $layout->getBlock('choose-payment-method')
		...
		if(Mage::helper('onestepcheckout')->isEnterprise()){
            $giftcardScripts = $layout->createBlock('enterprise_giftcardaccount/checkout_onepage_payment_additional', 'giftcardaccount_scripts', array('template'=>'onestepcheckout/giftcardaccount/onepage/payment/scripts.phtml'));
            $paymentMethod->append($giftcardScripts);
        }
        ...
        $result = array(
			...
			'payment_method' => $paymentMethod->toHtml(),
            'summary' => $output
