...
class Aitoc_Aitmanufacturers_Model_Mysql4_Aitmanufacturers_Collection extends Mage_Core_Model_Mysql4_Collection_Abstract
{
    ...
    {
        $this->getSelect()->join(
            array('sort_table' => $this->getResource()->getMainTable()),
            'main_table.id = sort_table.id',
            array('_sort' => 'IF(sort_table.sort_order > 0, sort_table.sort_order, 999999999)')
        )
        ->reset('order')
        ->order('_sort')
        ->order('manufacturer');
    ...
    protected function _getManufacturersCollection($storeId=null)
    {
        return Mage::getResourceModel('eav/entity_attribute_option_collection')
                ->setAttributeFilter(Mage::helper('aitmanufacturers')->getAttributeId($storeId))
                ->setStoreFilter(is_null($storeId)?Mage::app()->getStore():$storeId, true)
                ->setPositionOrder('desc', true);
    }
    
    public function getManufacturersCollection($filderIds = array(), $storeId = 0)
    {
    ...
    {
        $collection = $this->_getManufacturersCollection($storeId);
