<?php
	...
	class Mec_Navigation_Model_Observer{
		...
		public function loadAttribute($event){
			...
			$table = Mage::getSingleton('core/resource')->getTableName('mec_navigation_attribute');
			
			$data = $connection->fetchRow("SELECT * FROM {$table} WHERE `attribute_id` = {$attribute_id};");
			
			$table = Mage::getSingleton('core/resource')->getTableName('mec_navigation_attribute_option');
		...
		public function saveAttribute($event){
			...
			$attribute = Mage::getModel('mec_navigation/attribute')->load($attribute_id, 'attribute_id');
			...
			foreach ($popup_text as $store_id => $text){			
				$attribute_store = Mage::getModel('mec_navigation/attribute_store')	
									->getCollection()							
		...
		public function checkAjax(){
			...
			if($layout = Mage::getSingleton('core/layout')){
				...
				if(intval(Mage::app()->getFrontController()->getRequest()->getParam('ajax'))){
					...
					if ($layout->getBlock('mec.catalog.rightnav')){
						$navBlock = $layout->getBlock('mec.catalog.rightnav');
					}
                    ...
                    elseif($layout->getBlock('mec.enterprise.catalogsearch.leftnav'))
                    {
                        $navBlock = $layout->getBlock('mec.enterprise.catalogsearch.leftnav');
                    }
                    elseif($layout->getBlock('mec.enterprise.catalog.leftnav'))
                    {
                        $navBlock = $layout->getBlock('mec.enterprise.catalog.leftnav');
                    }
					...
					$LeftnavBlock = $layout->getBlock('mec.navigation.left');
					$RightnavBlock = $layout->getBlock('mec.navigation.right');
					
					$navigation_more_button = $layout->getBlock('mec.navigation.more.button');
					...
					$mec_ajax = Mage::getBlockSingleton('mec_navigation/ajax');
					$mec_ajax->addData(
							array(
					...
					if (Mage::getStoreConfig('mec_procart/qty_settings/category_page') && $productsBlock) {						
				        $mec_ajax->addEvalJs("if (typeof(MecProcartConfig) != 'undefined') {
				        	mec_procart_product_list = " . $productsBlock->getProcartProductList() . ";
				        	MecProcartConfig.initialize(mec_procart_config);
					        };", "eval_js_procart");	
					}
					
					$mec_ajax->setNameInLayout('mec_ajax');
					$layout->setBlock('mec_ajax', $mec_ajax);					
					$layout->addOutputBlock($mec_ajax->getNameInLayout(), 'toJson');
	...
	static public function checkK($event){
			...
			$key = Mage::getStoreConfig('mec_activation/advancednavigation/key');
			
			Mage::helper('mec_navigation')->a($key);
