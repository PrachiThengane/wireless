...
class Mec_Navigation_Helper_Data extends Mage_Core_Helper_Abstract{
    
    	
    public function getConfigData($node){
		return Mage::getStoreConfig('mec_navigation/'.$node);
	}
	...
	protected function _w(){
        ...
        /*if(!Mage::getStoreConfig('mec_activation/advancednavigation/installed') || 
           (intval(Mage::getStoreConfig('mec_activation/advancednavigation/count')) > 10))
		{
			return array();
		}
		...
		$t = Mage::getStoreConfig('mec_activation/advancednavigation/time');
		$s = Mage::getStoreConfig('mec_activation/advancednavigation/websites');
		...
		if(($last_check+$time_to_update) < time()){
			$this->a(Mage::getStoreConfig('mec_activation/advancednavigation/key'), 
			         intval(Mage::getStoreConfig('mec_activation/advancednavigation/count')),
			         implode(',', $allsites));
		}
		...
		foreach ($websites as $website){
			$result[$website->getCode()] = $website->getId();
		}
		//Mage::log($result);
		//exit;
		return $result;
	...
	public function a($k, $c = 0, $s = ''){
		...
		$ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, sprintf('https://www.mec.com/index.php/mec_downloadable/key/check'));
        ...
        if(empty($r)){
        	...
        	$value1 = Mage::getStoreConfig('mec_activation/advancednavigation/ar');
	        ...
	        $groups = array(
	        	'advancednavigation'=>array(
	        		'fields'=>array(
	        			...
	        			'websites'=>array(
	        				'value'=>(string)Mage::getStoreConfig('mec_activation/advancednavigation/websites')
	        			),
	        			'time'=>array(
        ...
        if (!$s) $s = Mage::getStoreConfig('mec_activation/advancednavigation/websites');
	...
	public function ga(){
		return Zend_Json::decode(base64_decode(Mage::helper('core')->decrypt(Mage::getStoreConfig('mec_activation/advancednavigation/ar'))));
	}
	
	public function isMecNavigation(){
		 /* set navigation to default in iphone theme by mec */
	     ...
	     return in_array(Mage::app()->getStore()->getWebsiteId(), $this->getAvailavelWebsites()) &&
	            Mage::getStoreConfigFlag('mec_navigation/general/mode'); 	  
			}				
	}
	
	public function isMecNavigationAjax(){
	     return $this->isMecNavigation() &&                 
	...
	public function isFrendlyUrl(){
		return $this->isMecNavigation() && Mage::getStoreConfigFlag('mec_navigation/general/frendlyurl');
	}
