...
class Mec_Navigation_Block_Adminhtml_Catalog_Product_Attribute_Edit_Tab_Main extends Mage_Adminhtml_Block_Catalog_Product_Attribute_Edit_Tab_Main
{
    /**
    ...
    protected function _prepareForm()
    {
        ...
        $field = $fieldset->addField('filter_type', 'select', array(
            ...
            'values'=>Mage::getModel('mec_navigation/adminhtml_system_config_source_filter_type_attribute')->toOptionArray(),
        ));
        ...
        $field->setData('after_element_html', '<script type="text/javascript">
        	...
        	if($("frontend_input").value != "price"){
    			...
    			for(var i = 0; i < options.length; i++){
    				...
    				if(e.value == '.Mec_Navigation_Model_Layer::FILTER_TYPE_INPUT.' || e.value == '.Mec_Navigation_Model_Layer::FILTER_TYPE_SLIDER.' || e.value == '.Mec_Navigation_Model_Layer::FILTER_TYPE_SLIDER_INPUT.' || e.value == '.Mec_Navigation_Model_Layer::FILTER_TYPE_INPUT_SLIDER.'){
        	...
        	Event.observe($("frontend_input"), "change", function(e){
        		...
        		if(this.value == "price"){
        			...
        			if(!is_price){
						...
						option.value = "'.Mec_Navigation_Model_Layer::FILTER_TYPE_INPUT.'";
						option.innerHTML = "'.$helper->__('Input').'";
						
						$("filter_type").appendChild(option);
						
						var option = document.createElement("option");
						option.value = "'.Mec_Navigation_Model_Layer::FILTER_TYPE_SLIDER.'";
						option.innerHTML = "'.$helper->__('Slider').'";
						
						$("filter_type").appendChild(option);
						
						var option = document.createElement("option");
						option.value = "'.Mec_Navigation_Model_Layer::FILTER_TYPE_SLIDER_INPUT.'";
						option.innerHTML = "'.$helper->__('Slider and Input').'";
						
						$("filter_type").appendChild(option);
												
						var option = document.createElement("option");
						option.value = "'.Mec_Navigation_Model_Layer::FILTER_TYPE_INPUT_SLIDER.'";
        	...
        	Event.observe("filter_type", "change", function(){
                    ...
                    if (value == '.Mec_Navigation_Model_Layer::FILTER_TYPE_IMAGE.'){
    				...
    				}else if (value == '.Mec_Navigation_Model_Layer::FILTER_TYPE_DEFAULT_INBLOCK.'){
    					for (var i = 0; i < elements.length; i++) {
                    		var id = elements[i]; 
                            if ($(id)){
                            	$(id).up("td").up("tr").hide();	
    				...
    				}else if (value == '.Mec_Navigation_Model_Layer::FILTER_TYPE_SLIDER_INPUT.' || value == '.Mec_Navigation_Model_Layer::FILTER_TYPE_INPUT_SLIDER.'){
    					for (var i = 0; i < elements.length; i++) {
                    		var id = elements[i]; 
                            if ($(id)){
                            	$(id).up("td").up("tr").hide();	
                ...
                function init_filter_type() {
                	Mec_Navigation_fireEvent($("filter_type"), "change");
                }
        ...
        $fieldset->addField('image_align', 'select', array(
            ...
            'values'=> Mage::getModel('mec_navigation/adminhtml_system_config_source_filter_image_align')->toOptionArray(),
        ));
