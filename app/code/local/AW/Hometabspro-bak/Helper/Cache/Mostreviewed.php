 ...
 */
class AW_Hometabspro_Helper_Cache_Mostreviewed extends AW_Hometabspro_Helper_Cache
{

	protected $_reviewSummaryT;
	protected $_reviewEntityT;
	protected $_reviewStatusT;
		...
		public function setCache($conf) {
			...
			$select = $this->_connection->select()
                            ->from(array('summary'=>$this->_reviewSummaryT),array('summary.entity_pk_value'))
			...
			$where = array_unique($this->_connection->fetchCol($select));
			 $this->_session->setHometabsProMostReviewed($where);
			 $this->_session->setHometabsProMostReviewedLimit($limit);
