...
class AW_Featured_Helper_Images extends Mage_Core_Helper_Abstract {
    ...
    public function getProductImage($product, $imageId = null, $size = 120) {
        if(!is_object($product))
            $product = Mage::getModel('catalog/product')->load($product);
        if($product->getData()) {
            if(!$imageId && $product->getData('small_image') && $product->getData('small_image') != 'no_selection') {
            ...
            if($imageId && ($_gImage = self::getGalleryImage($product, $imageId))) {
                $_imageUrl = Mage_Core_Model_Store::URL_TYPE_MEDIA.DS.self::getFolderName().DS.self::imageResizeRemote($_gImage->getData('url'), $imageId, $size, $size);
                if($_imageUrl)
