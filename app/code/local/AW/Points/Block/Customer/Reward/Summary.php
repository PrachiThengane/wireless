 ...
 */class AW_Points_Block_Customer_Reward_Summary extends Mage_Core_Block_Template {
    ...
    protected function _construct(){
        ...
        $this->setData('website', Mage::getSingleton('customer/session')->getCustomer()->getStore()->getWebsite());
        $this->addData(
    ...
    public function getPointsConvertedIntoMoney(){    
        ...
        if ($result>0)    $result = Mage::app()->getStore()->convertPrice($result, true);

        } catch (Exception $e) {

            Mage::helper('awcore/logger')->log($this,
                    Mage::helper('points')->__('Unable to convert points into currency to show up in summary section of user account page.'),
