...
function startLoadNavigationData(more_products){
	...
	if (more_products && typeof(gan_more_type_ajax) != 'undefined'){
		...
		loadinfo.innerHTML = '<img src="'+loadimage+'" />'+mec_navigation_loadinfo_text;
	...
	if(loadimagealign == 'bottom'){
		...
		loadinfo.innerHTML = mec_navigation_loadinfo_text+'<img src="'+loadimage+'" alt="" class="align-'+loadimagealign+'"/>';
	
	}else{
		
		loadinfo.innerHTML = '<img src="'+loadimage+'" alt="" class="align-'+loadimagealign+'"/>'+mec_navigation_loadinfo_text;
...
function popUp(rel_id){
	
            $('a-' + rel_id).up().addClassName('quickviewpro-block');
            Event.observe($('a-' + rel_id), 'click', function(event) {
                var pro_id = rel_id;
				 loader.show();
						...
						new Ajax.Request(url, {
							...
							onSuccess:  function(transport) {
								loader.hide();
									...
									showpop(pro_id, transport.responseText);
